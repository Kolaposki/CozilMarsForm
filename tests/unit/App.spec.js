...
describe("App", () => {
  ...
  it('validates the email field', async () => {
    ...
    const emailErrorMessage = wrapper.find('span[data-error-for="personalInformationEmail"]');
    expect(emailErrorMessage.text()).toBe('Value is not a valid email address');

    await emailInput.setValue('johndoe@example.com');
    await emailInput.trigger('blur');

    expect(wrapper.find('span[data-error-for="personalInformationEmail"]').exists()).toBe(false);
  });
  ...
  it('validates the phone field', async () => {
    ...
    const phoneErrorMessage = wrapper.find('span[data-error-for="personalInformationPhone"]');
    ...
    expect(wrapper.find('span[data-error-for="personalInformationPhone"]').exists()).toBe(false);
  });
  ...
  it('validates the nationality field', async () => {
    ...
    const nationalityErrorMessage = wrapper.find('span[data-error-for="personalInformationNationality"]');
    expect(nationalityErrorMessage.text()).toBe('Value is required');

    await nationalityInput.setValue('american');
    await nationalityInput.trigger('blur');

    expect(wrapper.find('span[data-error-for="personalInformationNationality"]').exists()).toBe(false);
  });
